...
property_last_updated_numerical <- rep(0, nrow(train))
for (i in 1:nrow(train)) {
  components <- unlist(strsplit(train[i, "property_last_updated"], split = " "))
...
train$property_last_updated_numerical <- property_last_updated_numerical

features <- c(features, "property_last_updated_numerical")

#
# property_amenities
#

head(train$property_amenities, 20)
all_amenities <- c()
for (i in 1:nrow(train)) {
  all_amenities <- c(all_amenities, unlist(strsplit(train[i, "property_amenities"], ", ")))
}

amenity_data <- data.frame(train$target)

for (amenity in unique(all_amenities)) {
  amenity_data[,amenity] <- rep(0, nrow(train))
  for (i in 1:nrow(train)) {
    if (amenity %in% unlist(strsplit(train[i, "property_amenities"], ", "))) {
      amenity_data[i, amenity] <- 1
    }
  }
}

data_matrix_amenities <- model.matrix(train.target ~ . - 1, data = amenity_data)
lambda_min_amenities <- cv.glmnet(data_matrix_amenities, amenity_data$train.target, alpha = 0)$lambda.min
model_amenities <- glmnet(data_matrix_amenities, amenity_data$train.target, alpha = 0, lambda = lambda_min_amenities)
coefs <- coef(model_amenities)
highest_coefs <- order(-abs(coefs))[2:11]

train <- cbind(train, amenity_data[, highest_coefs])

selected_amenities <- colnames(amenity_data)[highest_coefs]
features <- c(features, selected_amenities)
...
summary_list <- c("property_summary", "property_space", "property_desc",
                  "property_neighbourhood", "property_notes", "property_transit",
                  "property_access", "property_interaction", "property_rules")
source("Useful functions2.R")

# In a different R-file, it was determined that the following list of words can
# help determine the price of the property.
important_words_high <- c("gelegen", "vlakbij", "plenty", "hip", "deco",
                          "outside", "whole", "second", "hallway", "linens",
                          "won", "alone", "working", "forã", "hip", "another",
                          "soleil", "3th", "terras", "twin", "groenplaats",
                          "stib", "local", "bien")

important_words_low <- c("smartflats", "upon", "emergencies", "urgent",
                         "contact", "service", "prepare", "charge", "personal",
                         "forget", "spam", "box")

train$summary_score <- get_summary_score_3(train, important_words_high, important_words_low, summary_list)

features <- c(features, "summary_score")

# Also include property_sqfeet/property_type/property_zipcode?
features <- c(features, "property_sqfeet", "property_type", "property_zipcode")
...
train_preprocessed <- dplyr::select(train, all_of(keep))

colSums(is.na(train_preprocessed))

# Write final data set to csv
write.csv(train_preprocessed, "data/preprocessed_train2.csv", row.names = FALSE)
...
preprocess_data(validation, train, params_and_models, "preprocessed_validation2")
preprocess_data(test, train, params_and_models, "preprocessed_test2")

validation_preprocessed <- read.csv("data/preprocessed_validation2.csv", header=TRUE)
